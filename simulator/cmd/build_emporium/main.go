...
func main() {
	...
	recoverySimple := flag.Bool("recovery-simple", false,
		"Set target DB to SIMPLE recovery model before loading, so the "+
			"TABLOCK bulk-copy can be minimally logged (much faster) at "+
			"the cost of no point-in-time restore (fine for regenerable data).")
	...
	*tier = normalizeTier(*tier)
	...
	if *loadPostgres == "" && *initSchema && !*recoverySimple {
		fmt.Fprintf(os.Stderr, "WARNING: --init-schema without --recovery-simple — under the full "+
			"recovery model the TABLOCK bulk-copy load isn't minimally logged, so it runs much "+
			"slower and the transaction log grows large. Add --recovery-simple unless you need "+
