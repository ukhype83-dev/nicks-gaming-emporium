...
- **PostgreSQL reporting functions.** The full `rpt` reporting library (~190
  functions, including the deliberately-awful tuning-lab set — scalar-UDF
  ...
  one, then `FETCH` from the returned cursor; multi-result functions return
...
- **Prebuilt PostgreSQL dumps.** Custom-format `pg_restore` dumps (`.dump`) are
  now published per tier alongside the SQL Server backups, so every tier can be
  downloaded and restored on either engine. Each carries a SHA-256, and a
  restored database reproduces its tier's published fingerprint exactly. Restore
  with `pg_restore -j`, then `ANALYZE` (dumps don't carry planner statistics).
...
### Fixed
- **A PostgreSQL build now analyzes the whole database at the end.**
  `batch.usp_refresh_everything` runs `ANALYZE` across every table — OLTP, web, and
  the `dw` warehouse — at the end of the refresh, so a freshly built database is
  query-ready immediately rather than waiting on autovacuum to reach the OLTP and
  web tables. Postgres doesn't auto-collect statistics on a freshly bulk-loaded
...
- **PostgreSQL DW ETL analyzes the facts before it builds on them, not only at
  the end.** The end-of-run `ANALYZE` (above) still left the *intermediate*
  consumers — the rollups and the `sales_wide` wide table — reading the
  just-loaded fact tables with no statistics, so at the medium/large tiers
  `sales_wide` seq-scanned the whole fact table once per month (a multi-hour
  grind, multi-day at the large tier). `batch.usp_refresh_everything` now
  `ANALYZE`s the fact tables and dimensions immediately after the fact loads and
  before the rollup/`sales_wide` step, so those consumers get BRIN pruning. Data
  is unchanged — purely a build-performance fix.
- **PostgreSQL fact loads defer their primary key.** The `dw` fact loaders
  (`batch.usp_refresh_fact_*`) already dropped and rebuilt their *secondary* indexes
  around the month-by-month bulk load; they now do the same for the **primary key**. A
  fact's surrogate key (`transaction_line_id`, …) is assigned in OLTP build order, not
  time order, so with the PK live during the chronological load every row is a random
  insert into a btree that, at the large tier, dwarfs `shared_buffers` — which made the
  large-tier fact load decelerate into a multi-day grind. The PK is now dropped before
  the load and rebuilt once at the end as a sorted, parallel index build. This is purely
  a build-order change: the resulting data is identical (verified — full reconciliation
  passes and every fingerprint metric is unchanged).
