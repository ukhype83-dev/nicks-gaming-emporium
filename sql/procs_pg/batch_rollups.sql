...
BEGIN
    ...
    -- Analyze the fact tables + dimensions BEFORE the rollups and sales_wide run.
    -- Both consume the freshly bulk-loaded facts by date range, and Postgres has
    -- no statistics on a just-loaded table, so without this the planner ignores
    -- the BRIN indexes and each consumer seq-scans the whole fact table -- once
    -- per month for sales_wide, which turns the wide-table build into a
    -- multi-hour grind (a multi-day one at the large tier). COMMIT first: ANALYZE
    -- runs in its own transaction. (The end-of-run ANALYZE below still refreshes
    -- stats for sales_wide and the rollups themselves.)
    COMMIT;
    FOR r IN SELECT format('%I.%I', schemaname, tablename) AS t
             FROM pg_tables
             WHERE schemaname = 'dw'
               AND (tablename LIKE 'fact%' OR tablename LIKE 'dim%')
    LOOP
        EXECUTE 'ANALYZE ' || r.t;
    END LOOP;
    ...
    -- Refresh planner statistics across the WHOLE database so it's query-ready.
    -- Postgres doesn't auto-analyze a freshly bulk-loaded table the way SQL
    -- Server maintains stats: without this the OLTP/web tables have no stats
    -- until autovacuum catches up, and the warehouse's BRIN indexes on the
    -- date-ordered fact tables are never chosen (every date-range query
    -- seq-scans the whole fact table). ANALYZE runs in its own transaction,
    -- hence the COMMIT first.
    COMMIT;
    FOR r IN SELECT format('%I.%I', schemaname, tablename) AS t
