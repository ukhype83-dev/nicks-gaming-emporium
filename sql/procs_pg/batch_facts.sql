...
BEGIN
    ...
    IF p_full THEN
        ...
        -- Drop the PK too. It is a maintained btree during the chronological load, but
        -- transaction_line_id is assigned in OLTP build order (not time order), so the keys
        -- arrive scattered — a random insert into an index that dwarfs shared_buffers, which
        -- turns the large-tier load into a multi-day grind. Rebuilt once, sorted, after the loop.
        ALTER TABLE dw.fact_sales DROP CONSTRAINT IF EXISTS pk_fact_sales;
    ...
    IF p_full THEN
        -- Rebuild the PK once, as a sorted/parallel index build (uses maintenance_work_mem +
        -- max_parallel_maintenance_workers) instead of 2.1B random single-row inserts.
        CREATE UNIQUE INDEX pk_fact_sales ON dw.fact_sales(transaction_line_id);
        ALTER TABLE dw.fact_sales ADD CONSTRAINT pk_fact_sales PRIMARY KEY USING INDEX pk_fact_sales;
...
BEGIN
    ...
    IF p_full THEN
        ...
        ALTER TABLE dw.fact_tradein DROP CONSTRAINT IF EXISTS pk_fact_tradein;   -- same deferral as fact_sales
    ...
    END LOOP;
    IF p_full THEN
        CREATE UNIQUE INDEX pk_fact_tradein ON dw.fact_tradein(trade_in_item_id);
        ALTER TABLE dw.fact_tradein ADD CONSTRAINT pk_fact_tradein PRIMARY KEY USING INDEX pk_fact_tradein;
...
BEGIN
    ...
    IF p_full THEN
        ...
        ALTER TABLE dw.fact_web_activity DROP CONSTRAINT IF EXISTS pk_fact_web;   -- same deferral as fact_sales
    ...
    END LOOP;
    IF p_full THEN
        CREATE UNIQUE INDEX pk_fact_web ON dw.fact_web_activity(page_view_id);
        ALTER TABLE dw.fact_web_activity ADD CONSTRAINT pk_fact_web PRIMARY KEY USING INDEX pk_fact_web;
